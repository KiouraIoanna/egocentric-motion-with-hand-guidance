    ...
    for split in ["val"]:
        ...
        for seq_name, start_idx in input_examples:
            for jdx in range(1):
                ...
                if hand_guidance is not None:
                    ...
                    if cli_args.save_eval_npz:
                        save_path = os.path.join(
                            out_dir, f"{seq_name}_st{start_idx}_traj_align_eval.npz"
                        )
                        ...
                        np.savez_compressed(
                            ...
                            # Needed for 2D camera-plane plotting
                            gt_aria_traj_T=gt_aria_traj_T.detach().cpu().numpy(),

                            pred_left_wrist=pred_left_wrist,
                            pred_right_wrist=pred_right_wrist,
                            gt_left_wrist=gt_left_wrist,
                            gt_right_wrist=gt_right_wrist,
                ...
                save_video(
                    imgs[..., ::-1],
                    f"{split}_idx{idx}_{seq_name_b}_{st}_{en}_{task}_{jdx}_gt_uem_guided{hand_tag}",
                    out_dir,
                    fps=10,
                )
