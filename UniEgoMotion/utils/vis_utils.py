...
def visualize_sequence_blender(
    *,
    ...
    def get_point_mesh(points, radius=0.055):
        points = to_numpy(points)
        sphere = trimesh.creation.uv_sphere(radius=radius, count=[8, 8])
        verts = []
        faces = []
        for point in points:
            faces.append(sphere.faces + len(verts) * len(sphere.vertices))
            verts.append(sphere.vertices + point[None])
    ...
    pred_global_jpos_mesh = None
    if pred_global_jpos is not None:
        pred_global_jpos = to_numpy(pred_global_jpos)
        point_verts, point_faces = get_point_mesh(pred_global_jpos[0] - offset)
        pred_global_jpos_mesh = scene.renderables.add_mesh(
            point_verts,
            point_faces,
            material=material,
            colors=UniformColors(pred_joint_color),
        )
        nframes = resolve_nframes(nframes, len(pred_global_jpos))

        if tracking_camera:
            centers.append(to_numpy(pred_global_jpos.mean(1) - offset) * xy_mask)
    ...
    for frame in tqdm(range(nframes)):
        ...
        if pred_global_jpos is not None:
            point_verts, _ = get_point_mesh(pred_global_jpos[frame] - offset)
            pred_global_jpos_mesh.update_vertices(point_verts)
